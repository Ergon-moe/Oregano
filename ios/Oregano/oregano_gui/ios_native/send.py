...
from oregano.util import timestamp_to_datetime, NotEnoughFunds, ExcessiveFee
    ...
    def reader_didScanResult_(self, reader, result) -> None:
        ...
        if self.qrScanErr:
            if type(self.qrScanErr) is int and self.qrScanErr == 2:
                ...
                message = _("The QR code does not appear to be a valid ERG address or payment request.\nPlease try again.")
            reader.stopScanning()
            parent().show_error(
                title = title,
                message = message,
