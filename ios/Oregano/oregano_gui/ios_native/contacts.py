...
from oregano.util import timestamp_to_datetime
...
class NewContactVC(NewContactBase):
    ...
    def reader_didScanResult_(self, reader, result) -> None:
        ...
        if not Address.is_valid(result):
            ...
            message = _("The QR code does not appear to be a valid ERG address.\nPlease try again.")
            reader.stopScanning()
...
def pay_to(addys : list) -> bool:
    ...
    if len(addys) > 1:
        gui.ElectrumGui.gui.show_error(title=_("Coming Soon"),
                                       message=_("This version of Oregano currently only supports sending to 1 address at a time! Sorry!"))
        return False
