...
from oregano.bitcoin import TYPE_ADDRESS, TYPE_SCRIPT, int_to_hex, var_int
from oregano.i18n import _
from oregano.plugins import BasePlugin
...
from oregano.util import print_error, is_verbose, bfh, bh2u, versiontuple
...
MSG_NEEDS_FW_UPDATE_CASHADDR = _('Firmware version (or Ergon app) too old for CashAddr support. ') + \
        ...
        try:
            ...
            if (e.sw == 0x6faa):
                raise Exception(_("{hw_device_name} is temporarily locked - please unplug and plug it in again."
                                  "\n\nIf this problem persists please exit and restart the Ergon "
        ...
        if not self.preflightDone:
            try:
                ...
                if (e.sw == 0x6d00 or e.sw == 0x6700):
                    raise BaseException(_('{} not in Ergon mode').format(self.device)) from e
                raise e
