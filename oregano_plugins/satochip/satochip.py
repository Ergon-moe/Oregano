...
from oregano.bitcoin import TYPE_ADDRESS, int_to_hex, var_int
from oregano.i18n import _
from oregano.plugins import BasePlugin, Device
from oregano.keystore import Hardware_KeyStore
from oregano.transaction import Transaction
from oregano.wallet import Standard_Wallet
from oregano.util import print_error, bfh, bh2u, versiontuple, PrintError, is_verbose
from oregano.bitcoin import hash_160, Hash
from oregano.mnemonic import Mnemonic, Mnemonic_Electrum, seed_type_name, is_seed
from oregano.plugins import run_hook
from oregano.bitcoin import serialize_xpub
from oregano_gui.qt.qrcodewidget import QRCodeWidget, QRDialog #from electrum.gui.qt.qrcodewidget import QRCodeWidget, QRDialog
        ...
        for i,txin in enumerate(tx.inputs()):
            ...
            for j, x_pubkey in enumerate(x_pubkeys):
                ...
                if x_pubkey in derivations:
                    ...
                    #(response, sw1, sw2) = client.cc.card_parse_transaction(pre_tx, True) # use 'True' since XRG use BIP143 as in Segwit...
                    ...
                    (response, sw1, sw2, tx_hash, needs_2fa) = client.cc.card_parse_transaction(pre_tx, True) # use 'True' since XRG use BIP143 as in Segwit...
...
class SatochipPlugin(HW_PluginBase):
    ...
    def setup_device(self, device_info, wizard):
        ...
        client.cc.parser.authentikey_from_storage=None # https://github.com/simpleledger/Oregano-SLP/pull/101#issuecomment-561238614
        ...
        while(True):
            ...
            if  (client.cc.setup_done):
                ...
                if (v_supported<v_applet):
                    msg=(_('The version of your Satochip is higher than supported by Oregano. You should update Oregano to ensure correct functioning!')+ '\n'
